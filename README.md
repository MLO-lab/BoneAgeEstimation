...
-  [DeepTICy Structure-Based Evaluation](#deepticy-structure-based-evaluation)
-  [Workflow](#workflow)
...
--- 
## DeepTICy Structure-Based Evaluation

This repository also contains the structure-based evaluation framework used to assess predictions generated by **DeepTICy**. The main DeepTICy model implementation, including its training and inference code, is maintained in a separate GitHub repository and is not reproduced here.

The evaluation methodology and code in [`DeepTICy_structure_based_eval/`](DeepTICy_structure_based_eval/) were developed and implemented here. The framework compares predicted and ground-truth structures in 3D Imaris (`.ims`) images at multiple levels:

- **Topology-aware evaluation**: clDice-based comparison of skeletonized structures.
- **Surface-based evaluation**: Surface Dice (sfDice) with a physical-distance tolerance.
- **Object-level evaluation**: Centroid-based matching and confusion-matrix analysis for megakaryocytes and adipocytes.

### Evaluation Files

- **`eval_utils.py`**: Data loading, chunking, matching, and metric implementations.
- **`structure_based_eval_cldice.py`**: Command-line evaluation using clDice.
- **`structure_based_eval_sfdice.py`**: Command-line evaluation using surface Dice.
- **`MK_Adipocytes_object_eval.ipynb`**: Object-based evaluation of megakaryocytes and adipocytes.
- **`sbatch_*.sh`**: Example SLURM configurations used for arteries, capillaries, and sinusoids.

The command-line scripts generate summary CSV files, detailed JSON results, and metadata files containing the evaluation parameters.
